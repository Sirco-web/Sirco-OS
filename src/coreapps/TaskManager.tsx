...
class TaskManager extends App {
								...
								>
									<td>
										<span>
											...
											/>
											<span class="proc-title">{proc.deref()?.title}</span>
										</span>
									</td>
									...
									<td class="pid">
										<span class="pid">{proc.deref()?.pid}</span>
									</td>
								</tr>
							)),
						)}
	...
	async open(args: string[] = []): Promise<WMWindow | undefined> {
		...
		if ((performance as any).measureUserAgentSpecificMemory) {
			const measure = async () => {
				if (!(performance as any).measureUserAgentSpecificMemory) return {}; // this API only works on chromium
			...
			while (page.parentElement) {
				...
				const allRows = Array.from(
					(document.querySelectorAll("[id^=taskmgr-proc-]") as any).values(),
				);
				...
				for (const row of allRows) {
					if (row) {
						((row as HTMLElement).children[1]! as HTMLElement).innerText =
							"(System Process)";
					}
				}
			}
