    ...
    try {
      if (typeof $scramjetLoadController !== 'undefined') {
        ...
        scramjet = new ScramjetController({
          files: {
            wasm: 'https://browser-app.is-a.lol/scram/scramjet.wasm.wasm',
            all: 'https://browser-app.is-a.lol/scram/scramjet.all.js',
            sync: 'https://browser-app.is-a.lol/scram/scramjet.sync.js',
          },
        });
        scramjet.init();
      ...
      if (typeof BareMux !== 'undefined') {
        connection = new BareMux.BareMuxConnection("https://browser-app.is-a.lol/baremux/worker.js");
      }
    } catch (err) {
...
async function configureTransport(force = false) {
  ...
  try {
    const wispUrl = "wss://browser-app.is-a.lol/wisp/";
    
    if (!force) {
        const currentTransport = await connection.getTransport();
        if (currentTransport === "https://browser-app.is-a.lol/epoxy/index.mjs") return;
    }
    
    await connection.setTransport("https://browser-app.is-a.lol/epoxy/index.mjs", [{ wisp: wispUrl }]);
  } catch (err) {
...
// Sirco-OS uses anura-sw.js which is already registered at boot
// No need to register a separate service worker for the browser
// The external Scramjet server handles all proxy functionality
window.registerSW = async function() {
  // anura-sw.js already controls this page
