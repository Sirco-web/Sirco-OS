...
Virtio9p.prototype.ReceiveRequest = async function(bufchain) {
    ...
    switch(id) {
        ...
        case 116: // read
            ...
            // console.log("[read]: fid=" + fid + " offset=" + offset + " count=" + count);
            ...
            if(!fidata.rcache)
                fs.readFile(path, function(err, data) {
                    ...
                    fidata.rcache = {data: data, remaining: data.byteLength};
                    _read(data);
                });
            else {
        ...
        case 120: // clunk
            ...
            if(fidata.wops) {
                fs.readFile(fidata.path, (err, dat) => {
                    ...
                    // console.log("making buffer of size "+ length);
                    ...
                    if(err) {
                        ...
                        fs.writeFile(fidata.path, self.Buffer.from(data), (err) => {
                            if(err)
                                console.error(err);
                            delete self.fids[fid];
                            this.BuildReply(id, tag, 0);
